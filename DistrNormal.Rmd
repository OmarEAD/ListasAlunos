...
```{r fig-gauss, echo=FALSE, fig.cap='Representação de uma curva em forma de sino (a área sob a curva não é um).', tidy=FALSE}
x <- seq(-2,2,0.01)
a <- -2
b <- 0
c <- 1
...
O gráfico apresentado na Figura @fig-gauss não tem área sob a curva igual a um, mas tem o formado de sino equivalente à curva normal.
...
em que, completando quadrados na primeira exponencial da Equação (@eq-p1), temos
...
\end{equation*}
```
Agora precisamos que nossa $f(x)$ integrada nos intervalos de menos infinito a mais infinito seja igual a $1$, conforme Equação (@eq-normalizacao), ou seja,
...
\end{equation*}
```
Dessa forma, temos abaixo a função densidade de probabilidade ($fdp$), Equação (@eq-gaussiana), a qual queríamos demonstrar, ou seja,
...
\end{eqnarray*}
```
Para resolvermos esta integral, faremos uma mudança de variável para $x$, conforme Equações (@eq-mv1) e (@eq-mv2)), ficando com
...
\end{eqnarray*}
```
em que a primeira integral do lado direito da igualdade acima é igual a zero (ver seção @sec-intb).
...
\end{equation*}
```
conforme seção @sec-inta. Dessa forma, temos que
...
e fazendo a substituição da variável $x$ conforme Equações ((@eq-mv1)) e ((@eq-mv2)), ficamos com
...
\end{equation*}
```
que pode ser resolvida por partes, conforme seção @sec-intc, para tornar-se
